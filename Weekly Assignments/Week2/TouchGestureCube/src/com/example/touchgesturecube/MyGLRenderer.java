...
public class MyGLRenderer implements GLSurfaceView.Renderer{
    ...
    static float[] accumulatedRotation = new float[16];
    static float[] accumulatedScale = new float[16];
    public static float scale = 1.0f;
    public static int vXDir = 0;
    public static int vYDir = 0;
    public static float velocityX = 0.0f;
    public static float velocityY = 0.0f;
    public static float accelerationX = 0.01f;
    ...
    public void onSurfaceCreated(GL10 unused, EGLConfig config) {
        ...
        Matrix.setIdentityM(MyGLRenderer.accumulatedRotation, 0);
        
        Matrix.setIdentityM(MyGLRenderer.accumulatedScale, 0);
        mCube = new Cube();
    }
    ...
    public void onRotate(float dX, float dY){
    	float[] combinedRotation = new float[16];

    	Matrix.setIdentityM(combinedRotation, 0);
    	
    	Matrix.rotateM(combinedRotation, 0, dX, 0.0f, 1.0f, 0.0f);
    	Matrix.rotateM(combinedRotation, 0, dY, 1.0f, 0.0f, 0.0f);
    	
        float[] temp = new float[16];
        
        Matrix.multiplyMM(temp, 0, combinedRotation,
        			0, MyGLRenderer.accumulatedRotation, 0);  
        System.arraycopy(temp, 0, MyGLRenderer.accumulatedRotation, 0, 16);
    }
...
class Cube {
    ...
    private long time, previousTime, elapsedTime;
    ...
    public void transform() {
        ...
        //Create rotations for fling, using velocity and # rotations per second
    	float[] flingRotation = new float[16];
    	Matrix.setIdentityM(flingRotation, 0);
    	Matrix.rotateM(flingRotation, 0, MyGLRenderer.velocityX * MyGLRenderer.rotationsPerSecond, 
    			0.0f, 1.0f, 0.0f);
    	Matrix.rotateM(flingRotation, 0, MyGLRenderer.velocityY * MyGLRenderer.rotationsPerSecond, 
    			1.0f, 0.0f, 0.0f);
    	
    	//Every second change velocity by 1/10th of its current speed.
    	if(elapsedTime > 1000){
    		if(MyGLRenderer.vXDir == 1){
    			if(MyGLRenderer.velocityX > 0.01)
    				MyGLRenderer.velocityX -= MyGLRenderer.velocityX*MyGLRenderer.accelerationX;
    			else
    				MyGLRenderer.velocityX = 0;
    		}
    		else if (MyGLRenderer.vXDir == -1){
    			if(MyGLRenderer.velocityX < -0.01)
    				MyGLRenderer.velocityX += -1.0f*MyGLRenderer.velocityX*MyGLRenderer.accelerationX;
    			else
    				MyGLRenderer.velocityX = 0;
    		}

    		if(MyGLRenderer.vYDir == 1){
    			if(MyGLRenderer.velocityY > 0.01)
    				MyGLRenderer.velocityY -= MyGLRenderer.velocityY*MyGLRenderer.accelerationY;
    			else
    				MyGLRenderer.velocityY = 0;
    		}
    		else if (MyGLRenderer.vYDir == -1){
    			if(MyGLRenderer.velocityY < -0.01)
    				MyGLRenderer.velocityY += -1.0f*MyGLRenderer.velocityY*MyGLRenderer.accelerationY;
    			else
    				MyGLRenderer.velocityY = 0;
    		}
        ...
        Matrix.multiplyMM(temp, 0, flingRotation,
        			0, MyGLRenderer.accumulatedRotation, 0);  
        System.arraycopy(temp, 0, MyGLRenderer.accumulatedRotation, 0, 16);
    
        Matrix.scaleM(MyGLRenderer.modelMatrix, 0, MyGLRenderer.scale, MyGLRenderer.scale, MyGLRenderer.scale);
        // rotation (x and y axis)
        Matrix.multiplyMM(temp, 0, MyGLRenderer.modelMatrix, 0, MyGLRenderer.accumulatedRotation, 0);
        System.arraycopy(temp, 0, MyGLRenderer.modelMatrix, 0, 16);
